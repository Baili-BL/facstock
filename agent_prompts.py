...
### 硬性规则（违反则答案无效）
1. `recommendedStocks` 里每只股票的 **code（六位数字）必须与扫描表中某一行完全一致**，禁止 601012、300750 等常见示例股名，除非该代码真的出现在扫描表中。
2. `name`、`price`、`changePct`、`score`、`grade`、`sector` 必须与扫描表中对应代码的行一致，可微调 `buyRange`/`stopLoss`/`signal` 等建议字段。
3. `marketCommentary`、`positionAdvice`、`riskWarning` 必须是你**根据扫描数据与新闻自己写的**中文句子，**禁止**输出「不超过30字」「市场简评，不超过」等说明性占位句。
4. 若扫描数据中无符合策略的标的，`recommendedStocks` 可为 `[]`，并在 `marketCommentary` 中说明原因。

### JSON Schema（字段须齐全；字符串勿抄本说明里的任何示例句）
...
def build_user_prompt(agent: dict, scan_data: str, news_data: str, holdings_data: str,
                      current_time: str, scan_date: str = None) -> str:
...
def build_messages(agent: dict, scan_data: str, news_data: str, holdings_data: str,
                   current_time: str, scan_date: str = None):
    return [
        {"role": "system", "content": agent["system_prompt"]},
        {"role": "user", "content": build_user_prompt(agent, scan_data, news_data,
                                                       holdings_data, current_time, scan_date)},
    ]
...
def normalize_agent_stock_code(code: Optional[str]) -> str:
    """统一为 6 位数字代码，兼容 601012.SH / sh601012。"""
    if code is None:
        return ''
...
def text_looks_like_schema_echo(s: str) -> bool:
    if s is None or not str(s).strip():
        return True
    t = str(s).strip()
    return any(m in t for m in _SCHEMA_ECHO_MARKERS)


def _fallback_market_commentary(candidates: List[dict]) -> str:
    if not candidates:
    ...
    n = len(candidates)
    out = f'扫描候选{n}只，热点含「{top_sec}」等，样本涨跌均值约{avg:+.1f}%。'
    return out[:48]


def _fallback_position_advice(candidates: List[dict]) -> str:
    _ = candidates
    return '结合指数与个人风险承受力控仓，低吸不追高，设好止损。'


def _fallback_risk_warning() -> str:
    return '板块轮动快，信号滞后；勿满仓单票，严守纪律。'


def sanitize_parsed_agent_output(
    parsed: dict,
    ...
    *,
    ...
    """
    清洗模型输出：
    - 推荐股 code 必须在 candidates 中，否则丢弃；全无则用评分 TOP 兜底
    - 与扫描表冲突的 name/price 等以扫描数据为准
    - 照抄 Prompt 占位句的文案替换为基于扫描的兜底句
    """
    ...
    for c in candidates or []:
        k = normalize_agent_stock_code(c.get('code'))
        if k and len(k) == 6:
            by_code[k] = c
    ...
    for s in recs_in:
        if not isinstance(s, dict):
            continue
        k = normalize_agent_stock_code(s.get('code'))
    ...
    if not recs_out and candidates:
        ...
        for row in sorted(candidates, key=_score, reverse=True)[:max_recs]:
            k = normalize_agent_stock_code(row.get('code'))
            if not k:
                continue
            recs_out.append({
                'code': row.get('code'),
    ...
    if text_looks_like_schema_echo(str(parsed.get('marketCommentary', ''))):
        parsed['marketCommentary'] = _fallback_market_commentary(candidates or [])
    if text_looks_like_schema_echo(str(parsed.get('positionAdvice', ''))):
        parsed['positionAdvice'] = _fallback_position_advice(candidates or [])
    if text_looks_like_schema_echo(str(parsed.get('riskWarning', ''))):
        parsed['riskWarning'] = _fallback_risk_warning()
