...
function thresholdFreedmanDiaconis(values, min3, max3) {
  ...
  return c5 && d ? Math.ceil((max3 - min3) / (2 * d * Math.pow(c5, -1 / 3))) : 1;
}

// node_modules/d3-array/src/threshold/scott.js
function thresholdScott(values, min3, max3) {
  const c5 = count(values), d = deviation(values);
  return c5 && d ? Math.ceil((max3 - min3) * Math.cbrt(c5) / (3.49 * d)) : 1;
}
...
function areaRadial_default() {
  var a3 = area_default().curve(curveRadialLinear), c5 = a3.curve, x06 = a3.lineX0, x12 = a3.lineX1, y06 = a3.lineY0, y12 = a3.lineY1;
...
MonotoneX.prototype = {
  ...
  point: function(x4, y4) {
    ...
    switch (this._point) {
      ...
      case 2:
        this._point = 3;
        point4(this, slope2(this, t12 = slope3(this, x4, y4)), t12);
        break;
      default:
        point4(this, this._t0, t12 = slope3(this, x4, y4));
        break;
    }
    this._x0 = this._x1, this._x1 = x4;
    this._y0 = this._y1, this._y1 = y4;
...
function link_default(links) {
  var id = index2, strength = defaultStrength, strengths, distance = constant_default2(30), distances, nodes, count2, bias, random, iterations2 = 1;
  if (links == null) links = [];
...
function areaPoint(lambda, phi) {
  lambda *= radians, phi *= radians;
  phi = phi / 2 + quarterPi;
  var dLambda = lambda - lambda0, sdLambda = dLambda >= 0 ? 1 : -1, adLambda = sdLambda * dLambda, cosPhi = cos2(phi), sinPhi = sin2(phi), k2 = sinPhi0 * sinPhi, u = cosPhi0 * cosPhi + k2 * cos2(adLambda), v = k2 * sdLambda * sin2(adLambda);
...
function boundsRingEnd() {
  ...
  areaStream.lineEnd();
  if (abs2(deltaSum) > epsilon3) lambda02 = -(lambda1 = 180);
  range2[0] = lambda02, range2[1] = lambda1;
  p0 = null;
}
...
function centroidLinePoint(lambda, phi) {
  lambda *= radians, phi *= radians;
  var cosPhi = cos2(phi), x4 = cosPhi * cos2(lambda), y4 = cosPhi * sin2(lambda), z = sin2(phi), w = atan22(sqrt2((w = y0 * z - z0 * y4) * w + (w = z0 * x4 - x0 * z) * w + (w = x0 * y4 - y0 * x4) * w), x0 * x4 + y0 * y4 + z0 * z);
  W1 += w;
  X1 += w * (x0 + (x0 = x4));
  Y1 += w * (y0 + (y0 = y4));
  Z1 += w * (z0 + (z0 = z));
...
function centroidRingPoint(lambda, phi) {
  lambda *= radians, phi *= radians;
  var cosPhi = cos2(phi), x4 = cosPhi * cos2(lambda), y4 = cosPhi * sin2(lambda), z = sin2(phi), cx = y0 * z - z0 * y4, cy = z0 * x4 - x0 * z, cz = x0 * y4 - y0 * x4, m2 = hypot(cx, cy, cz), w = asin2(m2), v = m2 && -w / m2;
  X2.add(v * cx);
  Y2.add(v * cy);
  Z2.add(v * cz);
  W1 += w;
...
function centroid_default(object2) {
  ...
  if (m2 < epsilon22) {
    x4 = X1, y4 = Y1, z = Z1;
    if (W1 < epsilon3) x4 = X0, y4 = Y0, z = Z0;
    m2 = hypot(x4, y4, z);
    if (m2 < epsilon22) return [NaN, NaN];
  }
  return [atan22(y4, x4) * degrees, asin2(z / m2) * degrees];
}
...
function circleStream(stream, radius, delta, direction, t02, t12) {
  ...
  for (var point5, t = t02; direction > 0 ? t > t12 : t < t12; t -= step) {
...
function circle_default2() {
  var center = constant_default3([0, 0]), radius = constant_default3(90), precision = constant_default3(2), ring, rotate, stream = { point: point5 };
...
function polygonContains_default(polygon, point5) {
  ...
  var sum3 = new Adder();
  if (sinPhi === 1) phi = halfPi2 + epsilon3;
  else if (sinPhi === -1) phi = -halfPi2 - epsilon3;
...
function clip_default(pointVisible, clipLine, interpolate, start) {
  return function(sink) {
    ...
    function ringEnd() {
      ...
      ringSink.lineEnd();
      var clean = ringSink.clean(), ringSegments = ringBuffer.result(), i, n = ringSegments.length, m2, segment, point6;
      ring.pop();
      polygon.push(ring);
      ring = null;
      if (!n) return;
      if (clean & 1) {
        ...
        if ((m2 = segment.length - 1) > 0) {
          if (!polygonStarted) sink.polygonStart(), polygonStarted = true;
          sink.lineStart();
          for (i = 0; i < m2; ++i) sink.point((point6 = segment[i])[0], point6[1]);
          sink.lineEnd();
        }
        return;
      }
  ...
  return {
    ...
    point: function(lambda12, phi12) {
      ...
      if (abs2(delta - pi3) < epsilon3) {
        ...
        if (abs2(lambda04 - sign0) < epsilon3) lambda04 -= sign0 * epsilon3;
        if (abs2(lambda12 - sign1) < epsilon3) lambda12 -= sign1 * epsilon3;
  ...
  function clipLine(stream) {
    ...
    return {
      ...
      point: function(lambda, phi) {
        var point1 = [lambda, phi], point22, v = visible(lambda, phi), c5 = smallRadius ? v ? 0 : code(lambda, phi) : v ? code(lambda + (lambda < 0 ? pi3 : -pi3), phi) : 0;
...
function clipRectangle(x06, y06, x12, y12) {
  ...
  return function(stream) {
    ...
    function polygonInside() {
      var winding = 0;
      for (var i = 0, n = polygon.length; i < n; ++i) {
        for (var ring2 = polygon[i], j = 1, m2 = ring2.length, point6 = ring2[0], a0, a1, b0 = point6[0], b1 = point6[1]; j < m2; ++j) {
...
function lengthPoint(lambda, phi) {
  lambda *= radians, phi *= radians;
  var sinPhi = sin2(phi), cosPhi = cos2(phi), delta = abs2(lambda - lambda03), cosDelta = cos2(delta), sinDelta = sin2(delta), x4 = cosPhi * sinDelta, y4 = cosPhi02 * sinPhi - sinPhi02 * cosPhi * cosDelta, z = sinPhi02 * sinPhi + cosPhi02 * cosPhi * cosDelta;
...
var containsObjectType = {
  ...
  FeatureCollection: function(object2, point5) {
    var features = object2.features, i = -1, n = features.length;
    while (++i < n) if (containsGeometry(features[i].geometry, point5)) return true;
    return false;
  }
};
var containsGeometryType = {
  ...
  MultiPoint: function(object2, point5) {
    var coordinates2 = object2.coordinates, i = -1, n = coordinates2.length;
    while (++i < n) if (containsPoint(coordinates2[i], point5)) return true;
    return false;
  },
  ...
  MultiLineString: function(object2, point5) {
    var coordinates2 = object2.coordinates, i = -1, n = coordinates2.length;
    while (++i < n) if (containsLine(coordinates2[i], point5)) return true;
    return false;
  },
  ...
  MultiPolygon: function(object2, point5) {
    var coordinates2 = object2.coordinates, i = -1, n = coordinates2.length;
    while (++i < n) if (containsPolygon(coordinates2[i], point5)) return true;
    return false;
  },
  GeometryCollection: function(object2, point5) {
    var geometries = object2.geometries, i = -1, n = geometries.length;
    while (++i < n) if (containsGeometry(geometries[i], point5)) return true;
    return false;
  }
};
...
function interpolate_default(a3, b) {
  var x06 = a3[0] * radians, y06 = a3[1] * radians, x12 = b[0] * radians, y12 = b[1] * radians, cy0 = cos2(y06), sy0 = sin2(y06), cy1 = cos2(y12), sy1 = sin2(y12), kx0 = cy0 * cos2(x06), ky0 = cy0 * sin2(x06), kx1 = cy1 * cos2(x12), ky1 = cy1 * sin2(x12), d = 2 * asin2(sqrt2(haversin(y12 - y06) + cy0 * cy1 * haversin(x12 - x06))), k2 = sin2(d);
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a3 = a0 + a1, b = b0 + b1, c5 = c0 + c1, m2 = sqrt2(a3 * a3 + b * b + c5 * c5), phi2 = asin2(c5 /= m2), lambda22 = abs2(abs2(c5) - 1) < epsilon3 || abs2(lambda04 - lambda12) < epsilon3 ? (lambda04 + lambda12) / 2 : atan22(b, a3), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs2((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x06, y06, lambda04, a0, b0, c0, x22, y22, lambda22, a3 /= m2, b /= m2, c5, depth, stream);
...
function projectionMutator(projectAt) {
  ...
  function recenter() {
    var center = scaleTranslateRotate(k2, 0, 0, sx, sy, alpha).apply(null, project(lambda, phi)), transform = scaleTranslateRotate(k2, x4 - center[0], y4 - center[1], sx, sy, alpha);
...
function conicEqualAreaRaw(y06, y12) {
  ...
  if (abs2(n) < epsilon3) return cylindricalEqualAreaRaw(y06);
  var c5 = 1 + sy0 * (2 * n - sy0), r0 = sqrt2(c5) / n;
  function project(x4, y4) {
    var r = sqrt2(c5 - 2 * n * sin2(y4)) / n;
  ...
  project.invert = function(x4, y4) {
    ...
    return [l / n, asin2((c5 - (x4 * x4 + r0y * r0y) * n * n) / (2 * n))];
  };
  return project;
}
...
function azimuthalInvert(angle2) {
  return function(x4, y4) {
    var z = sqrt2(x4 * x4 + y4 * y4), c5 = angle2(z), sc = sin2(c5), cc = cos2(c5);
    return [
      atan22(x4 * sc, z * cc),
      asin2(z && y4 * sc / z)
    ];
...
function mercatorProjection(project) {
  ...
  function reclip() {
    var k2 = pi3 * scale(), t = m2(rotation_default(m2.rotate()).invert([0, 0]));
...
function identity_default4() {
  var k2 = 1, tx = 0, ty = 0, sx = 1, sy = 1, alpha = 0, ca, sa, x06 = null, y06, x12, y12, kx2 = 1, ky2 = 1, transform = transformer({
    point: function(x4, y4) {
...
function lab2(start, end) {
  var l = nogamma((start = lab(start)).l, (end = lab(end)).l), a3 = nogamma(start.a, end.a), b = nogamma(start.b, end.b), opacity = nogamma(start.opacity, end.opacity);
  return function(t) {
    start.l = l(t);
    start.a = a3(t);
    start.b = b(t);
...
function autoType(object2) {
  for (var key in object2) {
    var value = object2[key].trim(), number2, m2;
    if (!value) value = null;
...
//# sourceMappingURL=chunk-IHEPBTUF.js.map
